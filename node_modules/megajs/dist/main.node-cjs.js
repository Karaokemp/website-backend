    ...
    cs.request(req, (err, response) => {
      ...
      function handleMegaErrors(resp) {
        ...
        if (resp.statusCode === 509) {
          ...
          const error = Error('Bandwidth limit reached: ' + timeLimit + ' seconds until it resets'); // Export error as a property of the error so people don't need to parse the error message
...
class MutableFile extends File {
  ...
  _upload(opt, source, type, cb) {
    let encrypter = opt.uploadCiphertext ? through() : megaEncrypt(opt.key);
  ...
  _uploadAttribute(opt, source, type, cb) {
    const gotBuffer = (err, buffer) => {
      ...
      this._uploadWithSize(stream$$1, buffer.length, stream$$1, pause, type, opt.handle, cb);
    }; // handle buffer
  ...
  _uploadWithSize(stream$$1, size, source, pause, type, handle, cb) {
    const ssl = 0;
    const request$$1 = type === 0 ? {
      a: 'u',
      ssl,
      s: size,
      ms: 0,
    ...
    this.api.request(request$$1, (err, resp) => {
      if (err) return cb(err); // Those headers are forbidden, so let browsers use their defaults
